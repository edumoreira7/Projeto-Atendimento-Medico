...
void inserirHeap(heap *h, Registro *p) {
    if (h->qtde >= CAP) {
        printf("Capacidade máxima da fila prioritária atingida.\n");
        return;
    }

    int i = h->qtde++;
    while (i > 0 && h->dados[(i - 1) / 2].idade < p->idade) {
...
Registro removerHeap(heap *h) {
    Registro removido = h->dados[0];
    Registro temp = h->dados[--h->qtde];

    int pai = 0, filho = 1;
    while (filho < h->qtde) {
        if (filho + 1 < h->qtde && h->dados[filho + 1].idade > h->dados[filho].idade)
            filho++;

        if (temp.idade >= h->dados[filho].idade)
            break;

        h->dados[pai] = h->dados[filho];
        pai = filho;
        filho = 2 * pai + 1;
    }
    h->dados[pai] = temp;
...
void atendimentoP(heap *h, Lista *l) {
    while (1) {
        ...
        switch (opcao) {
            case 1: {
                Elista *paciente = procurarPaciente(l);
                if (paciente) {
                    inserirHeap(h, paciente->dados);
                }
                break;
            }
            case 2: {
                if (h->qtde > 0) {
                    Registro r = removerHeap(h);
                    printf("Paciente %s removido do atendimento prioritário.\n", r.nome);
                } else {
